---
title: "Risk sharing with limited commitoment and storage: Numerical solution"
...
Based on the modeling solution described above, I explain how to solve the model numerically.
Prepare $V_1^0(y, B, x)$, $V_2^0(y, B, x)$, $\underline{x}^0(y, B)$, $\overline{x}^0(y, B)$, and $B'^0(y, B, x)$, and below I describe the steps in $h$'th iteration.
...
relativeParetoWeightsBoundsArray[, 1,] <- max(relativeParetoWeightsGridPoints)
relativeParetoWeightsBoundsArray[, 2,] <- 1
relativeParetoWeightsBoundsArray[, 3,] <- min(relativeParetoWeightsGridPoints)
...
storageSeq[100:simSize] %>% 
  as_tibble %>% 
  mutate(period = seq(100, simSize)) %>% 
  ggplot(aes(x = period, y = value)) +
  geom_point() +
  xlab('Time period') +
  ylab('Public storage') +
  theme_classic()
consHH1Seq[100:simSize] %>%
  as_tibble %>% 
  mutate(period = seq(100, simSize)) %>% 
  ggplot(aes(x = period, y = value)) +
  geom_point() +
