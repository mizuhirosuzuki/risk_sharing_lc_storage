...
1. Assume that no participation constraint is binding, and solve for $B'$ so that the planner's Euler equation holds with equality (note that $\nu$ disappears since participation constraints do not bind):
$$
...
2. Next, I update $\overline{x}(y, B)$. For this, first solve for $x$ such that the household 1's participation constraint binds with 0 next-period saving:
$$
...
$$
3. To check if the planner's Euler equation is satisfied, calculate
$$
...
4. Next, I update $\underline{x}(y, B)$ in a similar way. First solve for $x$ such that the household 2's participation constraint binds with 0 next-period saving:
$$
...
$$
5. To check if the planner's Euler equation is satisfied, calculate
$$
...
6. For each $(y, B)$, for $x \in [\underline{x}(y, B), \overline{x}(y, B)]$, update values with
$$
\begin{aligned}
