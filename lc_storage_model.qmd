...
```{r}
storageGridPoints <- seq(0, sqrt(max(incomeGridPointsHH1) * 3), by = 1e-2)^2
...
calculateEulerEquationDiffWithBindingPC <- function(
    aggregateIncome,
    currentStorage,
  ...
  nu <- 1 - currentRelativeParetoWeight / previousRelativeParetoWeight
  # print(nu)
...
nextStorageArray <- array(
  0.5, dim = c(numStates, numRelativeParetoWeights, numStorageGridPoints)
  )
...
while ((diff > 1e-5) & (iter < 100)) {
  ...
  for (stateIndex in seq(1, numStates)) {
    ...
    for (storageIndex in seq(1, numStorageGridPoints)) {
      ...
      if (
        ...
        #   ) < 0
        # ) {
        #   relativeParetoWeightsLowerTmp <- max(relativeParetoWeightsGridPoints)
        # } else {
      ...
      if (relativeParetoWeightsLowerIndex <= relativeParetoWeightsUpperIndex) {
      ...
      for (weightIndex in seq(1, numRelativeParetoWeights)) {
        if (weightIndex > relativeParetoWeightsUpperIndex) {
          if (
            calculateEulerEquationDiffWithBindingPC(
              aggregateIncomeGridPoints[stateIndex],
              storageGridPoints[storageIndex],
              relativeParetoWeightsGridPoints[weightIndex],
          ...
          } else if (weightIndex < relativeParetoWeightsLowerIndex) {
            if (
              calculateEulerEquationDiff(
                aggregateIncomeGridPoints[stateIndex],
                storageGridPoints[storageIndex],
                # relativeParetoWeightsGridPoints[weightIndex],
                relativeParetoWeightsLower,
                returnOnStorage,
                0,
                aggregateIncomeGridPoints,
                relativeParetoWeightsGridPoints,
      ...
      for (weightIndex in seq(1, numRelativeParetoWeights)) {
        ...
        if (weightIndex < relativeParetoWeightsLowerIndex) {
          valueArrayHH1New[stateIndex, weightIndex, storageIndex] <- calculateValue(
            aggregateIncomeGridPoints[stateIndex],
            storageGridPoints[storageIndex],
            currentRelativeParetoWeight = relativeParetoWeightsLower,
            returnOnStorage,
            nextPeriodStorage = nextStorageArrayNew[stateIndex, weightIndex, storageIndex],
            relativeParetoWeightsGridPoints,
            valueArrayHH1,
          ...
          valueArrayHH2New[stateIndex, weightIndex, storageIndex] <- valueAutarkyHH2[stateIndex]
          
        } else if (weightIndex > relativeParetoWeightsUpperIndex) {
          valueArrayHH1New[
            stateIndex,
            relativeParetoWeightsGridPoints >= relativeParetoWeightsUpper,
            storageIndex
          ...
          valueArrayHH2New[stateIndex, weightIndex, storageIndex] <- calculateValue(
            aggregateIncomeGridPoints[stateIndex],
            storageGridPoints[storageIndex],
            currentRelativeParetoWeight = relativeParetoWeightsUpper,
            returnOnStorage,
            nextPeriodStorage = nextStorageArrayNew[stateIndex, weightIndex, storageIndex],
            relativeParetoWeightsGridPoints,
            valueArrayHH2,
      ...
      #   storageIndex
      # ] <- nextStorageArrayNew[
      #   stateIndex, relativeParetoWeightsLowerIndex, storageIndex 
      # ]
      # nextStorageArrayNew[
      #   stateIndex,
      #   relativeParetoWeightsGridPoints >= relativeParetoWeightsUpper,
      #   storageIndex
      # ] <- nextStorageArrayNew[
      #   stateIndex, relativeParetoWeightsUpperIndex, storageIndex 
      # ]
    }
    toc()
  }
  ...
  nextStorageArray[3,
                   ,
                   1] %>% plot 
  relativeParetoWeightsBoundsArray[2,3,] %>% plot
}

relativeParetoWeightsGridPoints[100]

valueArrayHH1[3,,100] %>% plot
valueArrayHH2[3,,100] %>% plot
valueArrayHH2[2,1,] %>% plot
relativeParetoWeightsBoundsArray[2,1,] %>% plot
...
nextStorageArray[1,
                 (
                   (relativeParetoWeightsBoundsArray[1,1,ii] <= relativeParetoWeightsGridPoints) &
                   (relativeParetoWeightsBoundsArray[2,1,ii] >= relativeParetoWeightsGridPoints)
                   ),
                 ii] %>% plot
nextStorageArray[1,
                 ,
                 1] %>% plot
relativeParetoWeightsGridPoints[51]
```

```{r, eval = FALSE}

incomeSeq <- sample(seq(1, numStates), size = 100, replace = TRUE)
...
for (i in seq(1, 100)) {
  storageSeq <- c(
    ...
    # nextStorageArray[
    #   incomeSeq[i],
    #   which.min(abs(relativeParetoWeightSeq[i] - relativeParetoWeightsGridPoints)),
    #   which.min(abs(storageSeq[i] - storageGridPoints))
    #   ]
  )
...
relativeParetoWeightsBoundsArray %>% max
relativeParetoWeightsGridPoints %>% max
